...
void CGameContext::SendChat(int ChatterClientID, int Team, const char *pText)
{
	...
	if (strcmp(helpcmd, pText) == 0) {
		SendChatTarget(ChatterClientID, "BombX is a kind of tag, where whoever is 'it' will explode if they don't tag someone first.");
		SendChatTarget(ChatterClientID, "You can tag or temporarily stun other players by hitting them.");
		SendChatTarget(ChatterClientID, "Can you survive the longest?");
		return; // don't need to announce to everyone that someone wanted to know more about the mode
	}
	...
	if(m_VoteCloseTime)
	{
		...
		else
		{
			...
			if(m_VoteUpdate)
			{
				...
				for(int i = 0; i < MAX_CLIENTS; i++)
				{
					if(!m_apPlayers[i] || m_apPlayers[i]->m_PreferredTeam == TEAM_SPECTATORS || aVoteChecked[i])	// don't count in votes by spectators
						continue;
...
void CGameContext::OnMessage(int MsgID, CUnpacker *pUnpacker, int ClientID)
{
	...
	if(Server()->ClientIngame(ClientID))
	{
		...
		else if(MsgID == NETMSGTYPE_CL_CALLVOTE)
		{
			...
			if(pPlayer->GetTeam() == TEAM_SPECTATORS && pPlayer->m_PreferredTeam == TEAM_SPECTATORS)
			{
		...
		else if (MsgID == NETMSGTYPE_CL_SETTEAM && !m_World.m_Paused)
		{
			...
			if(pMsg->m_Team == TEAM_SPECTATORS) {
				pPlayer->m_PreferredTeam = TEAM_SPECTATORS;
			}
			else
			{
				pPlayer->m_PreferredTeam = 0;
			}
			...
			{
				// Addition for bombX
				if (g_Config.m_SvSpectatorSlots == 0) {
					char aBuf[128];
					str_format(aBuf, sizeof(aBuf), "Only %d active players are allowed", Server()->MaxClients());
					SendBroadcast(aBuf, ClientID);
				}
				else
				{
					SendBroadcast("You will be added after the current round is over", ClientID);
				}
		...
		else if (MsgID == NETMSGTYPE_CL_CHANGEINFO)
		{
			...
			// Additions for BombX, to prevent player confusion.
			if (strncmp("bomb", pMsg->m_pSkin, 4) == 0)
			{
				str_copy(pPlayer->m_OriginalSkinName, g_Config.m_SvDefaultSkin, sizeof(pPlayer->m_OriginalSkinName));
			}
			else
			{
				str_copy(pPlayer->m_OriginalSkinName, pMsg->m_pSkin, sizeof(pPlayer->m_OriginalSkinName));
			}
			pPlayer->m_TeeInfos.m_UseCustomColor = pMsg->m_UseCustomColor;
			// This line is commented because coloration of tee body is used by the mode to represent information about the states of tees.
...
void CGameContext::OnInit(/*class IKernel *pKernel*/)
{
	...
	// select gametype
	if(str_comp(g_Config.m_SvGametype, "BombX") == 0)
		m_pController = new CGameControllerBOMBX(this);
	else
		m_pController = new CGameControllerBOMBX(this);
